...
namespace OSM2019.OSM
{
  abstract class OSMBase : I_OSM
  {
    ...
    public virtual void SetAgentNetwork(AgentNetwork agent_network)
    {
      ...
      this.SetLinkInfoValues(agent_network);
      return;
    }
    ...
    protected void SetLinkInfoValues(AgentNetwork agent_network)
    {
      ...
      foreach (var agent in agent_network.Agents)
      {
        ...
        {
          this.LinkInfoValues.Add(new LinkInfoValue(agent, neighbor_agent, this.LinkInfoValueWindowSize));
        }
      }
    }
    ...
    protected virtual void UpdateLinkInfoValues()
    {
      foreach (var link_info_value in this.LinkInfoValues)
      {
        //if (link_info_value.SourceAgent.IsSensor) continue;
        var received_sum_op = this.MyRecordRounds.Last().ReceiveOpinionsInRound[link_info_value.ReceiveAgent][link_info_value.SendAgent];
        var belief = link_info_value.ReceiveAgent.Belief;
    ...
    public virtual void PrintAgentInfo(Agent agent)
    {
      ...
      {
        Console.WriteLine($"- Weight: {link.AgentLinkID} Value {link.GetWeight(agent):f4}");
      }
